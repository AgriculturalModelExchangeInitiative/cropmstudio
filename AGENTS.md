...
After editing Python files (like `cropmstudio/routes.py`):

```bash
python -m py_compile cropmstudio/__init__.py  # Check single file for syntax errors
```
...
**Python** (in `cropmstudio/*.py` files):
...
- Backend processing in `cropmstudio/routes.py`
...
- **✅ Do**: `cropmstudio/` with underscores, all lowercase
...
```typescript
const PLUGIN_ID = 'cropmstudio:plugin';
```
...
export namespace CommandIDs {
  export const openPanel = 'cropmstudio:open-panel';
  export const refreshData = 'cropmstudio:refresh-data';
}
...
- Backend routes: `cropmstudio/routes.py` (class `DataRouteHandler`)
- Frontend logic: `src/` directory
- Python package: `cropmstudio/` directory
...
1. **Read the backend first** — Check `cropmstudio/routes.py` to understand the existing API contract
...
Create RESTful endpoints in `cropmstudio/routes.py`:
...
def setup_route_handlers(web_app):
    ...
    data_route = url_path_join(base_url, "cropmstudio", "data")
...
- Define matching endpoint path strings (e.g., `"hello"`, `"get-data"`) in both `cropmstudio/routes.py` and `src/api.ts` to ensure routes sync between backend and frontend
...
2. **Write the code** (TypeScript in `src/`, styles in `style/`, Python in `cropmstudio/`)
...
5. **Install the extension** (REQUIRED for JupyterLab to recognize it):
   ...
   jupyter server extension enable cropmstudio
   ```
...
- **Output**: Creates files in `lib/` and `cropmstudio/labextension/`
...
**After editing Python** (files in `cropmstudio/`):
...
- Ask user to search for the extension ID (`cropmstudio`) to see if it loaded
...
ls -la cropmstudio/labextension/  # Should contain bundled extension
```
...
- Backend routes: `cropmstudio/routes.py`
- Server extension config: `jupyter-config/server-config/cropmstudio.json`
...
**❌ Don't**: Write frontend interfaces without first checking the backend API contract in `cropmstudio/routes.py`
...
**❌ Don't**: Use absolute imports like `from cropmstudio.routes import ...`
...
- **Plugin ID** (in `src/index.ts`): `'cropmstudio:plugin'`
- **Command IDs** (in `src/commands.ts` or `src/index.ts`): `'cropmstudio:command-name'`
...
- **CSS classes** (in `style/index.css`): `.jp-cropmstudio-ClassName`
- **API routes** (in `cropmstudio/routes.py`): `url_path_join(base_url, "cropmstudio", "endpoint")`
