...
for the server extension and a NPM package named `cropmstudio`
...
# Change directory to the cropmstudio directory
...
jupyter server extension disable cropmstudio
pip uninstall cropmstudio
```

In development mode, you will also need to remove the symlink created by `jupyter labextension develop`
command. To find its location, you can run `jupyter labextension list` to figure out where the `labextensions`
folder is located. Then you can remove the symlink named `cropmstudio` within that folder.
...
```sh
pytest -vv -r ap --cov cropmstudio
```
