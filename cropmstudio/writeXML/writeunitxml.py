...
class writeunitXML():
    ...
    def _createInit(self):
        ...
        try:
            init = open("{0}{2}crop2ml{2}algo{2}pyx{2}init.{1}.pyx".format(self._datas['Path'], self._datas['Model name'].lower(), os.path.sep), 'w', encoding='utf8')
    ...
    def _createAlgo(self):
        ...
        try:
            algo = open("{0}{2}crop2ml{2}algo{2}pyx{2}{1}.pyx".format(self._datas['Path'], self._datas['Model name'].lower(), os.path.sep), 'w', encoding='utf8')
    ...
    def _write(self):
        ...
        try:
            if self._change_algo:
                open("{0}{2}crop2ml{2}algo{2}pyx{2}{1}.pyx".format(self._datas['Path'], self._datas['Model name'].lower(), os.path.sep), 'w', encoding='utf8').close()
            if self._change_init or ('init' in dir(self._df) and self._df["init"]):
                open("{0}{2}crop2ml{2}algo{2}pyx{2}init.{1}.pyx".format(self._datas['Path'], self._datas['Model name'].lower(), os.path.sep), 'w', encoding='utf8').close()
        except IOError as ioerr:
            # with self._out:
